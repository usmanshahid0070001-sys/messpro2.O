...
export default function BillEstimatorCard({
  ...
  // 1. Calculate Base Consumed Meal Cost from Real Records
  const { baseMealsCost, totalConsumedPortions } = useMemo(() => {
    let cost = 0
    let portions = 0
    records.forEach((r) => {
      if (r.attendance?.hasEaten) {
        const count = r.attendance.count || 1
        const price = r.mealInfo?.price || 0
        cost += price * count
        portions += count
      }
    })
    return { baseMealsCost: cost, totalConsumedPortions: portions }
  }, [records])
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground mt-0.5">
              Simulate your projected end-of-month hostel bill by adding custom charges, rent factors, and utility surcharges.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-card border border-border p-5 rounded-2xl shadow-xs space-y-3">
            <div className="flex items-center justify-between border-b border-border/60 pb-3">
              <div>
                ...
                <h4 className="text-base font-bold text-foreground mt-0.5">
                  Logged Dining Meals ({totalConsumedPortions} portions)
                </h4>
              </div>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1.5">
              ...
              <span>
                Computed in real-time from the sum of all confirmed meal attendances for {monthLabel}.
              </span>
            </p>
          </div>
