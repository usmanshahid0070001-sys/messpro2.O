...
const FormInput = ({ label, error, required, ...props }) => (
 ...
 <input
 {...props}
 className={`w-full px-3.5 py-2.5 bg-card border rounded-xl text-sm text-foreground placeholder:text-foreground dark:placeholder:text-foreground focus:outline-none focus:ring-1 transition-all ${
 error ?'border-red-400 focus:border-red-500 focus:ring-red-500':'border-border dark:border-border focus:border-border focus:ring-ring dark:focus:border-white dark:focus:ring-white'
 }`}
 />
...
export default function CreateHostelModal({ isOpen, onClose }) {
 ...
 className="relative w-full max-w-lg bg-background rounded-2xl shadow-2xl flex flex-col border border-border dark:border-border overflow-hidden"
 >
 {/* Header */}
 <div className="shrink-0 px-6 py-5 border-b border-border">
 <div className="flex items-start justify-between gap-4 mb-5">
 <div>
 <h2 className="text-lg font-bold text-foreground">Create New Hostel</h2>
 <p className="text-[13px] text-foreground dark:text-foreground mt-0.5">Step {step} of 3: {STEPS[step - 1].title}</p>
 </div>
 ...
 <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-[2px] bg-background dark:bg-background"/>
 <div 
 className="absolute left-0 top-1/2 -translate-y-1/2 h-[2px] bg-background dark:bg-white transition-all duration-300 ease-out"
 ...
 isActive ?'bg-background border-border dark:border-white text-foreground': 
 isCompleted ?'bg-background dark:bg-white border-border dark:border-white text-white dark:text-foreground': 
'bg-background border-border dark:border-border text-foreground dark:text-foreground'
 }`}>
 ...
 <div className="p-4 rounded-xl border border-border bg-background dark:bg-background">
 <h3 className="text-sm font-bold text-foreground mb-4">Admin Account</h3>
 <div className="grid grid-cols-2 gap-4">
 ...
 <div className="p-4 rounded-xl border border-border bg-background dark:bg-background">
 <h3 className="text-sm font-bold text-foreground mb-4">Manager Account</h3>
 <div className="grid grid-cols-2 gap-4">
 ...
 <div className="space-y-4">
 <div className="space-y-1.5 flex flex-col">
 <label className="block text-sm font-semibold text-foreground">Select a Plan <span className="text-red-500">*</span></label>
 <div className="relative">
 <select
 name="plan"value={formData.plan} onChange={handleChange} onBlur={handleBlur} disabled={loadingPlans}
 className={`w-full pl-3.5 pr-10 py-3 bg-card border rounded-xl text-sm text-foreground focus:outline-none focus:ring-1 appearance-none cursor-pointer ${errors.plan ?'border-red-400':'border-border dark:border-border'}`}
 >
 {loadingPlans ? <option value="">Loading...</option> : plans.map(p => (
 <option key={p._id} value={p._id}>{p.name} — ${p.price}/mo</option>
 ))}
 </select>
 <ChevronRight className="w-4 h-4 absolute right-3.5 top-1/2 -translate-y-1/2 text-foreground pointer-events-none rotate-90"/>
 </div>
 ...
 {/* Footer Actions */}
 <div className="shrink-0 px-6 py-4 border-t border-border bg-background/80 dark:bg-background/60 flex items-center justify-between">
 {step > 1 ? (
 <button type="button"onClick={handlePrev} disabled={loading} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-foreground dark:text-foreground hover:bg-background dark:hover:bg-background transition-colors">
 ...
 disabled={loading}
 className="px-4 py-2 rounded-xl text-sm font-semibold text-foreground hover:text-foreground dark:hover:text-white transition-colors"
 >
 Cancel
 </button>
 )}
 ...
 type="submit"
 disabled={loading}
 className="min-w-[132px] px-5 py-2.5 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:bg-black/80 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {step < 3 ? (
