...
const FormInput = ({ label, error, icon: Icon, required, ...props }) => (
 ...
 {Icon && <Icon className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-foreground dark:text-foreground"/>}
 <input
 {...props}
 className={`w-full ${Icon ?'pl-9':'px-3.5'} pr-3.5 py-2.5 bg-card border rounded-xl text-sm text-foreground placeholder:text-foreground dark:placeholder:text-foreground focus:outline-none focus:ring-1 transition-all ${error
 ?'border-red-400 focus:border-red-500 focus:ring-red-500'
 :'border-border dark:border-border focus:border-border focus:ring-ring dark:focus:border-white dark:focus:ring-white'
 }`}
 />
 </div>
...
const FeatureCard = ({ label, isChecked, onChange, disabled }) => (
 <label className={`relative flex items-center gap-3 cursor-pointer rounded-lg px-3 py-2.5 -mx-3 transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring dark:has-[:focus-visible]:ring-white ${isChecked ?'bg-secondary':'hover:bg-background dark:hover:bg-background'} ${disabled ?'opacity-60 cursor-not-allowed':''}`}>
 <div className={`w-[18px] h-[18px] shrink-0 rounded flex items-center justify-center transition-all duration-150 ${isChecked ?'bg-background dark:bg-white border border-border dark:border-white':'bg-card border border-border dark:border-border'}`}>
 {isChecked && <Check className="w-3 h-3 text-white dark:text-foreground"/>}
 </div>
 <input type="checkbox"className="sr-only"checked={isChecked} onChange={onChange} disabled={disabled} />
 <span className={`text-sm font-medium transition-colors ${isChecked ?'text-foreground':'text-foreground dark:text-foreground'}`}>
 {label}
 </span>
 </label>
);
...
export default function PlanFormModal({ isOpen, onClose, plan }) {
 ...
 className="relative w-full max-w-2xl max-h-[calc(100vh-3rem)] flex flex-col bg-background rounded-2xl shadow-2xl border border-border dark:border-border"
 >
 {/* Header */}
 <div className="shrink-0 flex justify-between items-start gap-4 px-6 py-5 border-b border-border">
 <div>
 <h2 className="text-lg font-bold text-foreground">
 {isEditing ?'Edit Plan':'Create New Plan'}
 </h2>
 <p className="text-[13px] text-foreground dark:text-foreground mt-0.5">
 {isEditing ?'Modify pricing, limits, and feature access.':'Define a new subscription tier for your tenants.'}
 </p>
 </div>
 <button onClick={onClose} className="p-2 rounded-lg text-foreground hover:text-foreground dark:hover:text-white hover:bg-accent transition-colors">
 <X className="w-5 h-5"/>
 </button>
 </div>
 ...
 <fieldset className="px-6 py-2 border-b border-border space-y-1">
 <legend className="text-[10px] font-bold uppercase tracking-widest text-foreground pt-4">Basic Info</legend>
 ...
 <label className="block text-sm font-semibold text-foreground">Description <span className="text-red-500">*</span></label>
 <span className="text-[11px] text-foreground">{formData.description.length}/240</span>
 </div>
 <textarea
 ...
 className={`w-full px-3.5 py-2.5 bg-card border rounded-xl text-sm transition-all resize-none ${errors.description ?'border-red-400':'border-border dark:border-border'}`}
 ...
 <div>
 <span className="text-sm font-semibold block text-foreground">Plan is Active</span>
 <span className="text-[11px] text-foreground">Inactive plans are hidden</span>
 </div>
 <ToggleSwitch checked={formData.isActive} onChange={() => setFormData(p => ({ ...p, isActive: !p.isActive }))} />
 </div>
 </fieldset>

 <fieldset className="px-6 py-2 border-b border-border">
 <legend className="text-[10px] font-bold uppercase tracking-widest text-foreground pt-4">Limits</legend>
 <div className="grid gap-4 sm:grid-cols-2">
 ...
 className="w-full px-3.5 py-2.5 bg-card border border-border dark:border-border rounded-xl text-sm text-white"
 />
 <p className="text-[11px] text-foreground">Use -1 for unlimited</p>
 </div>
 ))}
 </div>
 </fieldset>
 ...
 <fieldset className="px-6 py-2 border-b border-border">
 <legend className="text-[10px] font-bold uppercase tracking-widest text-foreground pt-4">Core Features (Included)</legend>
 ...
 <fieldset className="px-6 py-2 border-b border-border">
 <legend className="text-[10px] font-bold uppercase tracking-widest text-foreground pt-4">Attendance Methods</legend>
 <div className="grid gap-3 sm:grid-cols-2">
 ...
 <legend className="text-[10px] font-bold uppercase tracking-widest text-foreground pt-4">Other Services</legend>
 ...
 {/* Footer */}
 <div className="px-6 py-4 border-t border-border bg-background/80 dark:bg-background/60 flex justify-end gap-3">
 <button type="button"onClick={onClose} disabled={isLoading} className="px-4 py-2 rounded-xl text-sm font-semibold text-foreground hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
 Cancel
 </button>
 <button form={formId} type="submit"disabled={isLoading} className="min-w-[132px] px-5 py-2 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:bg-black/80 transition-colors flex items-center justify-center gap-2">
