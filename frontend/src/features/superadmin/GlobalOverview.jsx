...
export default function GlobalOverview() {
 ...
 >
 <div className="bg-background dark:bg-background rounded-[2rem] p-6 shadow-inner border border-border">
 <div className="bg-background rounded-2xl p-4 shadow-sm border border-border dark:border-border">
 ...
 fill="none"
 stroke="#dddddd"
 className="dark:stroke-border stroke-border"
 strokeWidth="4"
 strokeLinecap="round"
 />

 {/* x-axis labels (months) */}
 <g className="fill-foreground dark:fill-foreground font-bold text-[10px] uppercase tracking-widest"textAnchor="middle">
 ...
 {/* y-axis labels */}
 <g className="fill-foreground dark:fill-foreground font-bold text-[10px] uppercase tracking-widest"textAnchor="end">
 ...
 {planData.map((plan) => (
 <div key={plan.name}>
 <div className="mb-2 flex items-center justify-between gap-3 text-sm font-bold text-foreground dark:text-foreground">
 <span className="uppercase tracking-widest text-[10px]">{plan.name}</span>
 <span className="text-foreground">{plan.value} hostels</span>
 </div>
 <div className="h-2 rounded-full bg-secondary">
 <div className="h-2 rounded-full bg-background dark:bg-white"style={{ width: `${plan.percent}%` }} />
 </div>
 </div>
 ))}
 </div>
 ...
 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground dark:text-foreground">Platform Health</p>
 <div className="mt-5 space-y-4 text-sm font-bold text-foreground dark:text-foreground">
 ...
 <div className="flex items-center justify-between">
 <span>Suspended</span>
 <span className="text-foreground">1 tenant</span>
 </div>
 </div>
 </div>
 </SectionCard>
 ...
 action={<button className="text-xs font-black uppercase tracking-widest text-foreground dark:text-foreground hover:text-foreground dark:hover:text-white transition-colors">View all →</button>}
 >
 <div className="grid gap-3">
 {auditEvents.map((event) => (
 <div key={event.time} className="grid gap-3 rounded-2xl border border-border bg-card p-4 md:grid-cols-[100px_220px_1fr] items-center hover:bg-background dark:hover:bg-background transition-colors">
 <div className="font-mono text-[10px] font-bold text-foreground dark:text-foreground uppercase tracking-widest">{event.time}</div>
 <div className="text-xs font-black tracking-widest text-foreground uppercase">{event.code}</div>
 <div className="text-sm font-semibold text-foreground dark:text-foreground">{event.detail}</div>
 </div>
 ))}
 </div>
 </SectionCard>
