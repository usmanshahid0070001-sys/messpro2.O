export default function PageHeader({ 
  ...
  return (
    <header className="mb-8 glass-panel rounded-[2rem] p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 -mt-10 -mr-10 w-48 h-48 bg-slate-200/20 dark:bg-white/5 rounded-full blur-3xl pointer-events-none"></div>

      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 relative z-10">
        <div>
          {/* Top Badge */}
          <div className="flex items-center gap-2 mb-3">
            {Icon && <Icon className="w-4 h-4 text-slate-500 dark:text-[#888888]" />}
            <span className="text-[10px] font-black text-slate-500 dark:text-[#888888] uppercase tracking-[0.3em]">
              {badgeText}
            </span>
          </div>
          ...
          <p className="text-slate-600 dark:text-[#888888] font-semibold text-sm max-w-2xl mt-3">
            {subtitle}
          </p>
        </div>
