...
const ROLE_HIGHLIGHTS = {
  ...
  superadmin: {
    color: 'text-[#555] dark:text-[#aaa]',
    bg: 'bg-[#f5f5f5] dark:bg-[#1a1a1a] border-[#e5e5e5] dark:border-[#222]',
    label: 'SuperAdmin',
    points: [
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-white dark:bg-[#0a0a0a] rounded-2xl shadow-2xl ring-1 ring-black/5 flex flex-col border border-[#e5e5e5] dark:border-[#222] overflow-hidden max-h-[90vh]"
          >
            {/* ── Header ───────────────────────────────────────────────────── */}
            <div className="shrink-0 flex items-start justify-between gap-4 px-5 sm:px-6 py-4 sm:py-5 border-b border-[#f0f0f0] dark:border-[#1a1a1a]">
              <div className="flex items-center gap-3 min-w-0">
                <div className="shrink-0 w-9 h-9 bg-[#111] dark:bg-white rounded-xl flex items-center justify-center">
                  <Shield className="w-4 h-4 text-white dark:text-[#111]" />
                </div>
                <div className="min-w-0">
                  <h2 id="legal-modal-title" className="text-base font-bold text-[#111] dark:text-white">
                    Legal Agreement
                  </h2>
                  <p className="text-[12px] text-[#737373] dark:text-[#888] mt-0.5">
              ...
              {onClose && (
                <button
                  type="button"
                  onClick={onClose}
                  className="shrink-0 p-2 rounded-lg text-[#a3a3a3] hover:text-[#111] dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
                  aria-label="Close"
                >
                  <X className="w-4 h-4" />
                </button>
            ...
            {/* ── Role badge ───────────────────────────────────────────────── */}
            <div className="shrink-0 px-5 sm:px-6 py-3 border-b border-[#f0f0f0] dark:border-[#1a1a1a] bg-[#fafafa] dark:bg-[#0f0f0f]">
              <p className="text-[12px] text-[#555] dark:text-[#999]">
            ...
            >
              ...
              <div className={`border rounded-xl p-4 ${roleData.bg}`}>
                ...
                <ul className="space-y-2">
                  {roleData.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-sm text-[#444] dark:text-[#bbb] leading-relaxed">
              ...
              {/* Universal points */}
              <div className="border border-[#e5e5e5] dark:border-[#222] rounded-xl p-4">
                <p className="text-[11px] font-bold uppercase tracking-widest mb-3 text-[#a0a0a0] dark:text-[#555]">
                ...
                <ul className="space-y-2">
                  {[
                    ...
                    <li key={i} className="flex items-start gap-2.5 text-sm text-[#555] dark:text-[#999] leading-relaxed">
                      <span className="mt-1.5 shrink-0 w-1.5 h-1.5 rounded-full bg-[#d0d0d0] dark:bg-[#444]" />
                      {point}
                    </li>
                  ))}
                </ul>
              ...
              {!hasScrolledToBottom && (
                <motion.div
                  ...
                  className="flex flex-col items-center gap-1 py-2 text-[#c0c0c0] dark:text-[#444]"
                >
            ...
            {/* ── Footer ───────────────────────────────────────────────────── */}
            <div className="shrink-0 px-5 sm:px-6 py-4 border-t border-[#f0f0f0] dark:border-[#1a1a1a] bg-[#fafafa]/80 dark:bg-[#111]/60 backdrop-blur-sm space-y-3">
              {/* View Full Agreement */}
              <button
                type="button"
                onClick={handleViewFull}
                className="w-full flex items-center justify-center gap-2 text-sm font-medium text-[#737373] dark:text-[#888] hover:text-[#111] dark:hover:text-white transition-colors py-1"
              >
              ...
              <div className="flex gap-3">
                {onClose && (
                  <button
                    ...
                    onClick={onClose}
                    disabled={isAgreeing}
                    className="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-[#404040] dark:text-[#ddd] hover:bg-[#e5e5e5] dark:hover:bg-[#222] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    Decline
                  </button>
                )}
                <button
                  ...
                  className="flex-1 px-5 py-2.5 rounded-xl text-sm font-bold bg-[#111] dark:bg-white text-white dark:text-[#111] hover:bg-black/80 dark:hover:bg-white/90 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#111] dark:focus-visible:ring-white focus-visible:ring-offset-2"
                >
                  {isAgreeing ? (
                    <>
              ...
              {!hasScrolledToBottom && (
                <p className="text-[11px] text-center text-[#c0c0c0] dark:text-[#444]">
